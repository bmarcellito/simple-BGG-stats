...
def calculate_basics_plays(df_plays: pd.DataFrame, df_game_infodb: pd.DataFrame, toggle_expansion: bool) -> (
        pd.DataFrame):
    if len(df_plays) == 0:
        return pd.DataFrame()
    ...
    df_result = df.groupby("date", sort=True).sum().reset_index()
    df_result = df_result[["date", "quantity"]]

    while True:
        date_to_check = f'{start_year}-{start_month}'
        if date_to_check not in df_result["date"].values:
            next_line = {"date": [date_to_check], "quantity": 0}
    ...
    df_window = df_result["quantity"].rolling(window=12, min_periods=1).mean()
    df_result.columns = ["index", "Date", "Monthly plays"]
    df_result = pd.concat([df_result, df_window], axis=1)
    df_result.columns = ["index", "Period", "Monthly plays", "Averaged monthly plays"]
    return df_result
